 ...
 *   2. Redis 记忆（treehole:{userId}，最近 20 条，TTL 7 天）
 *
...
public class LangChain4jTreeHoleService {
    ...
    /** 记忆键前缀 */
    private static final String MEMORY_KEY_PREFIX = "treehole:";
    /** 记忆保留条数（最近 20 条 = 10 轮） */
     ...
     * 树洞对话
     *
     * @param userId  用户 ID（记忆键 treehole:{userId}）
     * @param message 用户消息
     * @return AI 回答
     */
    public String chat(Long userId, String message) {
        ...
        try {
            ...
            String answer = llmInvoker.invoke(config, messages, 0.85, "treehole",
    ...
    private List<LLMMessage> loadHistory(String redisKey) {
        try {
            ...
            log.warn("[LangChain4j-TreeHole] 读取记忆失败 key={} error={}", redisKey, e.getMessage());
            return new ArrayList<>();
        }
    }
    ...
    private void saveHistory(String redisKey, List<LLMMessage> history) {
        try {
            ...
            log.warn("[LangChain4j-TreeHole] 保存记忆失败 key={} error={}", redisKey, e.getMessage());
        }
    }
}
