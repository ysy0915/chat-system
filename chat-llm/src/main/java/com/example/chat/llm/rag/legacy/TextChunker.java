...
public class TextChunker {
     ...
     * 按默认参数分片
     */
    public List<LegacyVectorStoreService.ChunkText> chunk(String text) {
     ...
     * @param overlap 重叠字符数
     */
    public List<LegacyVectorStoreService.ChunkText> chunk(String text, int chunkSize, int overlap) {
        ...
        List<LegacyVectorStoreService.ChunkText> chunks = new ArrayList<>();
        ...
        for (String sentence : sentences) {
            ...
            if (trimmed.length() > chunkSize) {
                if (current.length() > 0) {
                    chunks.add(new LegacyVectorStoreService.ChunkText(current.toString(), chunkIndex++));
                    current = new StringBuilder();
                }
                for (int i = 0; i < trimmed.length(); i += chunkSize - overlap) {
                    int end = Math.min(i + chunkSize, trimmed.length());
                    chunks.add(new LegacyVectorStoreService.ChunkText(trimmed.substring(i, end), chunkIndex++));
                }
                continue;
            }
