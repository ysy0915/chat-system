...
# 步骤1：杀所有 Java 进程
echo "[1/5] 杀死所有 Java 进程..."
pkill -9 -f java 2>/dev/null
sleep 3

# 步骤2：等待端口完全释放
echo "[2/5] 等待端口释放..."
for port in 9090 8081 8082 8083 8084; do
...
# 步骤4：启动 chat-web 双实例
echo "[4/5] 启动 chat-web (8081/8082)..."
...
# 步骤5：启动 games + media + 守护进程
echo "[5/5] 启动 games (8083) + media (8084) + daemon..."
...
echo "===== 重启完成 $(date) ====="
echo "服务状态："
for svc in "core:9090" "web1:8081" "web2:8082" "games:8083" "media:8084"; do
    name=${svc%%:*}
    port=${svc##*:}
