 ...
 *
 * 图式工作流：
 *   START → pro（正方）→ con（反方）→ shouldContinue
 *                                    ├─ "pro" → pro（循环）
 *                                    └─ "summary" → summary → END
 */
@Service
...
public class DebateGraphService {
    ...
    public DebateState execute(String reqId, Long userId, String topic) throws Exception {
        ...
        DebateNodes nodes = new DebateNodes(llmInvoker, broadcastService,
                proModel, conModel, summaryModel, defaultApiKey);
        ...
        log.info("[DebateGraph] 辩论完成 proRounds={} conRounds={} summaryLen={}",
    ...
    private StateGraph<DebateState> buildGraph(DebateNodes nodes) throws Exception {
        ...
        graph.addNode("pro", AsyncNodeAction.node_async(nodes.proNode()));
        graph.addNode("con", AsyncNodeAction.node_async(nodes.conNode()));
        ...
        graph.setEntryPoint("pro");

        graph.addEdge("pro", "con");
        graph.addEdge("con", "shouldContinue");
        ...
        graph.addConditionalEdges("shouldContinue", AsyncEdgeAction.edge_async(condition),
                Map.of("pro", "pro", "summary", "summary"));
